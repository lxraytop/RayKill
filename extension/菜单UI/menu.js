...
window.nmimport(function(lib, game, ui, get, ai, _status){	
	ui.create.menu = function (connectMenu) {
		...
		var openMenu = function(node, e, onclose) {
			...
			/*game.playAudio('../extension/菜单UI/audio/Menu.mp3');
			/*这里也是第一次点击选项的声音*/
			popupContainer.onclose = onclose;
		};
		var clickToggle = function() {
		game.playAudio('../extension/菜单UI/audio/MidButton.mp3');
		/*关闭选项的声音*/
		...
		var clickSwitcher = function() {
			...
			game.playAudio('../extension/菜单UI/audio/WinButton.mp3');
			/*内部选项第一下点击的声音*/
			if (this._link.menu) {
		...
		var clickMenuItem = function() {
			...
			game.playAudio('../extension/菜单UI/audio/TinyButton.mp3');
			/*内部选项，比如斗地主选模式等*/
			if (config.onclick) {
		...
		(function() {
			...
			for (var i in lib.extensionMenu) {
				...
				game.editExtension = function(name) {
					page.currentExtension = name || '角完扩展';
				...
				var processExtension = function(exportext) {
					...
					setTimeout(function() {
						...
						page.currentExtension = inputExtName.value || '角完扩展';
		...
		(function() {
			...
			ui.click.consoleMenu = function() {
				...
				(function() {
					...
					function() {
						window.open('https://github.com/lxraytop/raykill');
					});
				...
				store.openCursor().onsuccess = function(e) {
					...
					if (cursor) {
						...
						saveButton.listen(function() {
							var current = this.parentNode.querySelector('.videonode.active');
							if (current) {
								game.export(lib.init.encode(JSON.stringify(current.link)), '角完杀 - 录像 - ' + current.link.name[0] + ' - ' + current.link.name[1]);
							}
						});
