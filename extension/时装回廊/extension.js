game.import("extension", function (lib, game, ui, get, ai, _status) {
  ...
  window.qhlyUI.assets = {
    huanpifu: {
      name: '../../../时装回廊/assets/huanpifu',
    },
    pinzhi: {
      name: '../../../时装回廊/assets/SF_pifu_pinzhiUI',
    },
    huanfu: {
      name: '../../../时装回廊/assets/huanfu',
    },
  }
  //出框调整
  //皮切用自己函数播放出框
  ...
  window.qhly_extension_package = {
    ...
    name: "时装回廊", content: function (config, pack) {
      //查看是否存在某扩展，用来处理兼容事宜
      ...
      lib.arenaReady.push(function () {
        ...
        if (!window.decadeUI) {
          let viewConfig = lib.config['extension_时装回廊_qhly_currentViewSkin'];
          if ((viewConfig == 'decade' || viewConfig == 'shousha') && !lib.config['extension_时装回廊_qhly_decadeCloseDynamic']) {
            let cfm = confirm("时装回廊：检测到UIPro并未正常开启，无法正常使用时装回廊中的十周年和手杀主题的动皮功能，点击【确定】将关闭所有动皮功能，点击【取消】将为您切换至默认主题并重启。");
            if (cfm) {
              game.saveConfig('extension_时装回廊_qhly_decadeCloseDynamic', true);
            } else {
              lib.config.qhly_currentViewSkin = 'shuimo';
              game.saveConfig('qhly_currentViewSkin', lib.config.qhly_currentViewSkin);
              game.saveConfig('extension_时装回廊_qhly_currentViewSkin', lib.config.qhly_currentViewSkin);
              game.reload();
            }
          }
        } else {
          ...
          if (!lib.config.qhly_mentionDynamic2) {
            let viewConfig = lib.config['extension_时装回廊_qhly_currentViewSkin'];
            if ((viewConfig == 'decade' || viewConfig == 'shousha') && lib.config['extension_时装回廊_qhly_decadeCloseDynamic']) {
              if (confirm("时装回廊：检测到您开启了时装回廊设置中的“关闭所有动皮效果”，这将无法正常使用时装回廊的动皮功能（并有可能造成其它错误），点击【确定】关闭该选项，点击【取消】将不再提醒。")) {
                game.saveConfig('extension_时装回廊_qhly_decadeCloseDynamic', false);
              } else {
            ...
            skinSwitch.dynamic.transformDst = function (player, isPrimary, dstInfo, extraParams = { isOrigin: false, huanfuEffect: null }) {
              ...
              if (dstInfo.name == null || dstInfo.name === avatar.name) {
                ...
                if (dstInfo.background) {
                  player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + dstInfo.background + '")';
                }
            ...
            EpicFX.playDynamic = (player, character, character2, play, transform, cutdybg) => {
              ...
              if (showDynamic && _status.mode != null) {
                ...
                for (var i = 0; i < avatars.length; i++) {
                  ...
                  // if (get.mode() == 'guozhan' && lib.config['extension_时装回廊_qhly_guozhanDS']) {
                  ...
                  else if (lib && lib.config && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && lib.config.extensions && lib.config.extensions.contains('时装回廊') && lib.config['extension_时装回廊_enable']) {
                    skin = null;
                  ...
                  if (lib.config['extension_时装回廊_qhly_editDynamic'] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin]['player'] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin]['player'][editArgument1] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin]['player'][editArgument1][theme]) {
                  ...
                  player.playDynamic(skinCopy, i == 1, cutdybg, lib.config['extension_时装回廊_ignoreClips']);
                  ...
                  // player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background + '")';
                  if (!skinCopy.dynamicBackground) player.$dynamicWrap.style.backgroundImage = `url(${lib.assetURL}extension/UIPro/assets/dynamic/${skinCopy.background}`;
                  if (!increased) {
                    increased = true;
        ...
        if (window.decadeUI && !lib.config['extension_时装回廊_qhly_decadeCloseDynamic'] && (lib.config.qhly_currentViewSkin == 'decade' || lib.config.qhly_currentViewSkin == 'shousha')) {
              ...
              init: {
                ...
                set: function (d) {
                  ...
                  if (newInit.indexOf('playDynamic') == -1) this.qhly_init = d;
                    ...
                    if (confirm("时装回廊：检测到有扩展修改武将登场动皮播放，这将与时装回廊“十周年”或“手杀”样式冲突，点击【确定】为您切换为其他样式。若点击【取消】，将不再对此消息进行提示。")) {
                      lib.config.qhly_currentViewSkin = 'shuimo';
                      game.saveConfig('qhly_currentViewSkin', lib.config.qhly_currentViewSkin);
                      game.saveConfig('extension_时装回廊_qhly_currentViewSkin', lib.config.qhly_currentViewSkin);
                      game.reload();
      ...
      function qhly_init(character, character2, skill) {
        ...
        if (window.qhly_newDynamicExt && lib.config['extension_EpicFX_skinEffects']) {
          ...
          if (showDynamic && _status.mode != null) {
            ...
            for (var i = 0; i < avatars.length; i++) {
              ...
              if (lib && lib.config && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && lib.config.extensions && lib.config.extensions.contains('时装回廊') && lib.config['extension_时装回廊_enable']) {
                skin = null;
                var namex = avatars[i];
                var value = game.qhly_getSkin(namex);
              ...
              var theme = ui.arena.dataset.newDecadeStyle == 'on' ? 'decade' : 'shousha';
              if (lib.config['extension_时装回廊_qhly_editDynamic'] && lib.qhly_skinEdit[realName] && lib.qhly_skinEdit[realName][skinName] && lib.qhly_skinEdit[realName][skinName].player && lib.qhly_skinEdit[realName][skinName].player.dynamic && lib.qhly_skinEdit[realName][skinName].player.dynamic[theme]) {
              ...
              this.playDynamic(skinCopy, i == 1, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on', lib.config['extension_时装回廊_ignoreClips']);
              // 修改3 start  此处修改是因为动皮和背景加载需要时间, 在动皮加载好之前用一个默认背景代替, 防止武将框黑一片
              if (i == 0 || !this.dynamic.primary) {
                if (skinCopy.background) {
                  this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background + '")';
                } else {
        ...
        this.node.avatar.addEventListener(lib.config.touchscreen ? "touchend" : "mouseup", function (e) {
          ...
          timer = setTimeout(function () {
            if (lib.config['extension_时装回廊_qhly_playerwindow']) game.qhly_playerWindow(this);
          }.bind(this), 220);
        });
      ...
      function qhly_reinit(from, to, maxHp, online) {
        ...
        if (!window.qhly_newDynamicExt) {
          ...
          if (this.doubleAvatar) {
            ...
            if (skin) {
              this.playDynamic(skinCopy, deputy, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on', lib.config['extension_时装回廊_ignoreClips']);
              decadeUI.CUR_DYNAMIC++;
            }
            var forces = 'qun';
            ...
            if (skin) {
              this.playDynamic(skinCopy, false, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on', lib.config['extension_时装回廊_ignoreClips']);
              decadeUI.CUR_DYNAMIC++;
              this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skin.background + '")';
            }
            var forces = 'qun';
      ...
      var qhly_mvp = function (result) {
        if (!lib.config['extension_时装回廊_qhly_mvp']) return;
      ...
      game.qhly_getDengJie = function (name) { //十周年样式中通过game.getRarity获取css中的属性等阶
        ...
        if (!lib.config['extension_时装回廊_qhly_decadeDengjie'] || lib.config['extension_时装回廊_qhly_decadeDengjie'] == 'auto') {
        ...
        else playerDengjie = lib.config['extension_时装回廊_qhly_decadeDengjie'];
      ...
      game.qhly_changeDynamicSkin = function (str, name, character, character2, play) {          //str主体，如果为字符串是名称中带有str的所有玩家，name指定更换为某种动皮，character玩家姓名id，character2是否为副将
        ...
        if (lib.config['extension_时装回廊_qhly_decadeCloseDynamic']) return;
        let res2;
        if (window.qhly_newDynamicExt) {
          res2 = {
            status: false,
        ...
        for (var dy = 0; dy < nodes.length; dy++) {
          ...
          if (_status.mode != null) {
            ...
            if (window.qhly_newDynamicExt) {
              ...
              node.playDynamic(skinCopy, bool2, cutdybg, lib.config['extension_时装回廊_ignoreClips']);
              node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background + '")';
              game.qhly_checkYH(node);
              // player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background + '")';
              if (!skinCopy.dynamicBackground) { } node.$dynamicWrap.style.backgroundImage = `url(${lib.assetURL}extension/UIPro/assets/dynamic/${skinCopy.background}`;
              if (!increased) {
                increased = true;
                decadeUI.CUR_DYNAMIC++;
              }
              ...
              if (skinCopy.background) {
                if (node.doubleAvatar && ui.arena.dataset.newDecadeStyle == 'on') {
                  ...
                  if (character2) {
                    background1 = node.dynamic.primary && node.dynamic.primary != null && node.dynamic.primary.background ? (lib.assetURL + 'extension/UIPro/assets/dynamic/' + node.dynamic.primary.background) : '';
                    background2 = lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background;
                  } else {
                    background1 = lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background;
                    background2 = node.dynamic.deputy && node.dynamic.deputy != null && node.dynamic.deputy.background ? (lib.assetURL + 'extension/UIPro/assets/dynamic/' + node.dynamic.deputy.background) : '';
                  }
                ...
                else node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/UIPro/assets/dynamic/' + skinCopy.background + '")';
              } else {
      ...
      lib.element.player.playChangeSkinEffect = function (avatar2) {
        ...
        var huanfuType = lib.config['extension_UIPro_newDecadeStyle'] == 'on' ? 'huanpifu' : 'huanfu';
        ...
        let bool1 = lib.config['extension_UIPro_newDecadeStyle'] == 'on' ? true : false;
        if (this.doubleAvatar) {
      ...
      game.qhly_checkYH = function (player, group) {
        ...
        if (lib.config['extension_UIPro_newDecadeStyle'] == "on") return;
      ...
      game.playShoushaAvatar = function (node, flip, name) {
        if (!lib.config['extension_时装回廊_qhly_shoushaTexiao'] || !window.decadeUI) return;
        ...
        if (game.qhly_hasExtension('皮肤切换') && lib.config[skinSwitch.configKey.useDynamic]) {
          node.isQhlx = true // 表示当前动皮角色是角完雷修版本的
      ...
      window.qhly_openPluginWindow = function(){
        ...
        var qhv = win.qhv;
        qhv.button.setBackgroundImage('extension/时装回廊/image/qhly_ok2.png');
      ...
      game.qhly_formatDS = function (obj, namex) {
        if (!lib.config['extension_时装回廊_qhly_formatDS'] || obj._hasFormated) return obj;
      ...
      if (lib.config.change_skin) {
        ...
        alert("请注意：本扩展功能将替代角完杀原生的换肤设置，为你带来更优秀的角色信息浏览体验。");
      }
      ...
      if (game.getFileList && lib.config.qhly_lihuiSupport) {
        game.qhly_checkFileExist('extension/时装回廊/lihui', function (s) {
          ...
          if (s) {
            game.getFileList('extension/时装回廊/lihui', function (folders, files) {
              if (files) {
      ...
      if (get.mode() == 'guozhan' && game.getFileList && lib.config.qhly_guozhan !== false) {
        var cpath = lib.config.qhly_originSkinPath == 'extension/时装回廊/sanguolutouskin/' ? 'extension/时装回廊/sanguolutouskin' : 'extension/时装回廊/sanguoskin';
        ...
        game.qhly_checkFileExist('extension/时装回廊/sanguoaudio', function (s) {
          if (s) {
            game.getFileList('extension/时装回廊/sanguoaudio', function (folders, files) {
      ...
      game.qhly_handleRect = function (rect) {
        if (game.qhly_hasExtension('UIPro')) return rect;
        return {
      ...
      game.qhly_parseConfig = function (obj) {
        ...
        var str = "";
        var image = obj.image ? obj.image : "extension/时装回廊/image/qhly_pic_config.png";
        ...
        characterLihui: function (name, skin) {
          ...
          if (skin) {
            if (lib.qhly_lihui[name + '-' + skin]) {
              return 'extension/时装回廊/lihui/' + lib.qhly_lihui[name + '-' + skin];
            }
            if (!lib.config.qhly_nolihuiOrigin) return null;
          }
          if (lib.qhly_lihui[name]) {
            return 'extension/时装回廊/lihui/' + lib.qhly_lihui[name];
          }
          return null;
        },
        prefix: 'image/character/',//武将原图在image/character内
        skin: {
          standard: (lib.config.qhly_originSkinPath ? lib.config.qhly_originSkinPath : 'extension/时装回廊/sanguoskin/'),//皮肤图片在时装回廊扩展内的位置。
          origin: 'extension/时装回廊/sanguoyuanhua/',//皮肤原画在时装回廊扩展内的位置。
        },
        //ssborder: 'shousha/',//手杀边框
        audioOrigin: '',
        audio: 'extension/时装回廊/sanguoaudio/',//皮肤配音文件在时装回廊扩展内的位置。
      };
      ...
      lib.qhly_groupimage['wei'] = 'extension/时装回廊/image/name_wei.webp';
      lib.qhly_groupimage['shu'] = 'extension/时装回廊/image/name_shu.webp';
      lib.qhly_groupimage['wu'] = 'extension/时装回廊/image/name_wu.webp';
      lib.qhly_groupimage['qun'] = 'extension/时装回廊/image/name_qun.webp';
      lib.qhly_groupimage['jin'] = 'extension/时装回廊/image/name_jin.webp';
      lib.qhly_groupimage['shen'] = 'extension/时装回廊/image/name_shen.webp';
      lib.qhly_groupimage['daqin'] = 'extension/时装回廊/image/name_daqin.webp';
      lib.qhly_groupimage['key'] = 'extension/时装回廊/image/name_key.webp';
          ...
          buttons[i].addEventListener(lib.config.touchscreen ? 'touchstart' : 'mousedown', function () {
            ...
            switch (this.id) {
              ...
              case 'qhly_bigedit6': {
                ...
                game.qhly_readFileAsText("extension/时装回廊/data/skineditmodel.txt", function (ret, str) {
                  if (ret) {
                    str = str.replace("_REPLACE_OBJECT_", strobj);
                    game.qhly_writeTextFile(str, "extension/时装回廊", "skinEdit.js", function (err) {
                      if (!err) {
                        console.log("保存成功");
                      } else {
      ...
      lib.extensionMenu['extension_时装回廊']['qhly_currentViewSkin'] = {
        "name": "UI套装",
        "intro": "设置UI套装样式。",
        "item": get.qhly_viewSkinSet(),
        "init": lib.config.qhly_currentViewSkin === undefined ? 'shuimo' : lib.config.qhly_currentViewSkin,
        onclick: function (item) {
          ...
          game.saveConfig('extension_时装回廊_qhly_currentViewSkin', item);
      ...
      lib.extensionMenu['extension_时装回廊']['qhly_currentMusic'] = {
        "name": "设置BGM",
      ...
      game.qhly_playQhlyAudio = function (name, num, repeat) {
        ...
        if (!repeat) {
          if (num === undefined || num === null) {
            game.playAudio('..', 'extension', '时装回廊', 'audio', name);
          } else {
            game.playAudio('..', 'extension', '时装回廊', 'audio', name + Math.ceil(Math.random() * num));
          }
        } else {
          if (num === undefined || num === null) {
            game.qhly_playAudioRepeatable('..', 'extension', '时装回廊', 'audio', name);
          } else {
            game.qhly_playAudioRepeatable('..', 'extension', '时装回廊', 'audio', name + Math.ceil(Math.random() * num));
          }
        }
      };
      ...
      game.qhly_refresh = function (name) {
        ...
        for (var player of players) {
          ...
          var finish = function (bool, fakeavatar) {
            ...
            //if (bool && lib.config['extension_时装回廊_qhly_decadeChangeEffect'] && (lib.config.qhly_smallwindowstyle == 'decade' || lib.config.qhly_smallwindowstyle == 'shousha')) player.playChangeSkinEffect(avatar == player.node.avatar2)
            ...
            if (lib.config.qhly_skinset.skin[cname]) {
              ...
              game.qhly_checkFileExist(destpath, function (s) {
                if (s) {
                  ...
                  else that.qhly_origin_setBackgroundImage('extension/时装回廊/image/noSkin.png');//noskin
                }
              })
      ...
      game.qhly_foundPackageExt = function (name) {
        ...
        if (cp) {
          ...
          return {
            ...
            audio: ((lib.config.qhly_extSkinPath == undefined || lib.config.qhly_extSkinPath == 'default') ? 'extension/' + extNameInPic + '/skin/audio/' : 'extension/时装回廊/sanguoaudio/'),//皮肤配音文件在时装回廊扩展内的位置。
          };
        }
        return null;
      };
        ...
        var state = {
          ...
          refreshLocation: function (view) {
            ...
            if (skin == currentSkinId || (!skin && !currentSkinId)) {
              view.setBackgroundImage('extension/时装回廊/theme/shuimo/newui_skin_background_shuimo.png');
            } else {
        ...
        try {
          ...
          autoskin.listen(function () {
            ...
            if (lib.config.qhly_autoChangeSkin == 'close' || !lib.config.qhly_autoChangeSkin) {
              open = true;
              item = lib.config['extension_时装回廊_qhly_decadeAuto'];
            }
            game.saveConfig('extension_时装回廊_qhly_autoChangeSkin', item);
          ...
          var playZhuDynamic = lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'always' ? false : true;
          game.qhly_getSkinList(namex, function (ret, list) {
            ...
            if (window.decadeUI) {
              ...
              if (dynamicSkinList) {
                if (lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'three' && dynamicSkinList.length > 3) playZhuDynamic = false;
            ...
            for (let i = 0; i < skinList.length; i++) {
              ...
              if (skin) {
                ...
                if (singleDynamic.contains(str) && lib.config['extension_时装回廊_qhly_dom2image']) skinView.toImageBtn.setAttribute('single', true);//打开快照
              ...
              if (level) {
                ...
                var img = null;
                if (map[level]) {
                  img = "extension/时装回廊/theme/decade/dc_" + map[level] + ".png";
                } else if (level.indexOf("#") == 0) {
                  var l2 = level.replace("#", "");
                  img = "extension/时装回廊/image/" + l2 + ".png";
                } else if (level.indexOf("$") == 0) {
                  var l2 = level.replace("$", "");
                  img = l2;
                }
              ...
              (function (name, skin, view) {
                view.listen(function () {
                  ...
                  game.qhly_setCurrentSkin(namex, skin, function () {
                    ...
                    //}
                    if (lib.config['extension_时装回廊_qhly_decadeChangeEffect'] && cPlayer) cPlayer.playChangeSkinEffect(false);
                    game.qhlySyncConfig();
                    if (lib.config.qhly_smallwinclosewhenchange) {
                      exitListener();
                    }
              ...
              if (skin) {
                ...
                game.qhly_checkFileExist(file, function (s) {
                  if (s) {
                    ...
                    else skinView2.qhly_origin_setBackgroundImage('extension/时装回廊/image/noSkin.png');//noskin
                  }
                })
              } else {
          ...
          if (cPlayer && cPlayer.name2) {
            ...
            var playFuDynamic = lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'always' ? false : true;
            game.qhly_getSkinList(namey, function (ret, list) {
              ...
              if (window.decadeUI) {
                ...
                if (dynamicSkinList) {
                  if (lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'three' && dynamicSkinList.length > 3) playFuDynamic = false;
                  dynamicSkinList.forEach(function (value, index, array) {
                    array[index] += '.jpg';
                  })
                  skinList = Array.from(new Set(skinList.concat(dynamicSkinList)));
                ...
                (function (name, skin, view) {
                  view.listen(function () {
                    ...
                    game.qhly_setCurrentSkin(namey, skin, function () {
                      ...
                      //}
                      if (lib.config['extension_时装回廊_qhly_decadeChangeEffect'] && cPlayer) cPlayer.playChangeSkinEffect(true);
                      game.qhlySyncConfig();
                      if (lib.config.qhly_smallwinclosewhenchange) {
                        exitListener();
                      }
          ...
          game.qhly_getSkinList(namex, function (ret, list) {
            ...
            for (let i = 0; i < skinList.length; i++) {
              ...
              if (level) {
                ...
                var img = null;
                if (map[level]) {
                  img = "extension/时装回廊/image/" + map[level] + ".png";
                } else if (level.indexOf("#") == 0) {
                  var l2 = level.replace("#", "");
                  img = "extension/时装回廊/image/" + l2 + ".png";
                } else if (level.indexOf("$") == 0) {
      ...
      ui.qhly_initCheckBox = function (view, checked, forbid) {
        ...
        if (view.qhly_checked) {
          view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxCheckedImage, 'extension/时装回廊/image/newui_checkbox_checked.png');
        } else {
          view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxImage, 'extension/时装回廊/image/newui_checkbox_unchecked.png');
        }
        view.qhly_setChecked = function (checked, trigger) {
          ...
          if (checked != this.qhly_checked) {
            ...
            if (this.qhly_checked) {
              this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxCheckedImage, 'extension/时装回廊/image/newui_checkbox_checked.png');
            } else {
              this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxImage, 'extension/时装回廊/image/newui_checkbox_unchecked.png');
            }
      ...
      game.qhly_initDecadeView = function (name, view, page, cplayer) {
        ...
        subView.page = {
              ...
              for (let skill of viewSkill) {
                ...
                if (detail) {
                  (function (skill) {
                    ...
                    if (img) {
                      img.src = lib.assetURL + get.qhly_getCurrentViewSkinValue('skillPagePlayAudioButtonImage', 'extension/时装回廊/image/newui_playaudio.png');
          ...
          skin: {
            ...
            editOpen: function (name, skin, skill, state) {
              ...
              game.qhly_editDialog(title, detail, initValue, function (value, dialog) {
                ...
                game.qhly_readFileAsText("extension/时装回廊/data/skininfomodel.txt", function (ret, str) {
                  if (ret) {
                    str = str.replace("_REPLACE_OBJECT_", strobj);
            ...
            refreshAfterGot: function (name, state) {
              ...
              var playBigDynamic = lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'always' ? false : true;
              if (this.dynamicSkinMore && lib.config['extension_时装回廊_qhly_decadeDynamic'] == 'three') playBigDynamic = false;
              if (this.firstRefresh) {
                ...
                for (var i = 0; i < this.skinList.length; i++) {
                  ...
                  if (this.skinList[i].single && lib.config['extension_时装回廊_qhly_dom2image']) skinView.toImageBtn.setAttribute('single', true);//6
          ...
          config: {
            ...
            init: function (name, state) {
              ...
              var that = this;
              var content = "";
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('favouriteImage', 'extension/时装回廊/image/newui_fav.png') + "' style='width:50px;margin-bottom:-4px;'/>收藏设置</h2>";
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('forbidImage', 'extension/时装回廊/image/newui_forbid.png') + "' style='width:50px;margin-bottom:-4px;'/>禁用设置</h2>";
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('rankImage', 'extension/时装回廊/image/newui_rank_icon.png') + "' style='width:50px;margin-bottom:-4px;'/>等阶设置</h2>";
              ...
              if (lib.config.qhly_enableCharacterMusic) {
                content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('musicImage', 'extension/时装回廊/image/newui_music_icon.png') + "' style='width:50px;margin-bottom:-4px;'/>音乐设置</h2>";
        ...
        //subView.pageButton[page].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/时装回廊/newui_button_selected.png'));
        ...
        refreshRank = function () {
          if (subView.rank) {
            if (lib.config.qhly_showrarity) {
              ...
              if (rarity) {
                subView.rank.setBackgroundImage('extension/时装回廊/theme/decade/rarity_' + game.getRarity(name) + '.png');
              }
            } else {
              subView.rank.hide();
            }
          ...
          let getTranslation = (name)=>{
            if(!get.slimNameHorizontal && pattern!='raw'){
              if(!lib.config.qhly_metioned_slimName){
                let r = prompt("你的角完杀版本暂不支持前缀文字显示，已经为你显示为原本的get.translation方式。点击“确认”不再提示此消息。");
                if(r){
      ...
      game.qhly_initNewView = function (name, view, page, cplayer) {
        ...
        if (currentViewSkin.isQiLayout) {
          ...
          if (lib.config.qhly_hideShuimoCover) {
            subView.cover.setBackgroundImage('extension/时装回廊/theme/shuimo/newui_shuimo_bg3.png');
          } else {
            subView.cover.setBackgroundImage('extension/时装回廊/theme/shuimo/newui_shuimo_bg2.png');
          }
        ...
        subView.page = {
            ...
            refreshAfterGot: function (name, state) {
                  ...
                  if (this.getSkinAt(i)) {
                    ...
                    if (currentViewSkin.isQiLayout) {
                      if (currentSkin == this.getSkinAt(i).skinId) {
                        currentSkinView.qhBoard.setBackgroundImage('extension/时装回廊/theme/shuimo/newui_skin_background_shuimo.png');
                    ...
                    if (level) {
                      ...
                      var img = null;
                      if (map[level]) {
                        img = "extension/时装回廊/image/level_" + map[level] + ".png";
                      } else if (level.indexOf("#") == 0) {
                        var l2 = level.replace("#", "");
                        img = "extension/时装回廊/image/" + l2 + ".png";
                      } else if (level.indexOf("$") == 0) {
              ...
              if (currentViewSkin.isQiLayout) {
                ...
                for (var audio of currentSkin.audios) {
                  ...
                  content += "<td>";
                  content += "<img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/时装回廊/image/qhly_pic_playaudiobutton.png') + "' style='height:40px;' id='qhly_skin_skill_" + audio.id + "'/>"
                  if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
                ...
                for (var audio of currentSkin.audios) {
                  ...
                  content += "】"
                  content += "&nbsp;&nbsp;&nbsp;<img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/时装回廊/image/qhly_pic_playaudiobutton.png') + "' style='height:23px;vertical-align:middle;' id='qhly_skin_skill_" + audio.id + "'/>"
                  if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
                    content += "<img src='" + lib.qhly_path + "image/qhly_editButton.png' style='height:25px;width:25px;'";
                    content += " id='qhly_skin_skill_edit_" + audio.id + "'";
                    content += "/>";
          ...
          config: {
            ...
            init: function (name, state) {
              ...
              var that = this;
              var content = "";
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('favouriteImage', 'extension/时装回廊/image/newui_fav.png') + "' style='width:50px'/>收藏设置</h2>";
              content += "<p>可以选择收藏此武将。进行自由选将操作时，可以更快找到此武将。</p>";
              content += "<p><span style='display:inline-block;height:30px;'><img id='qhconfig_checkbox_fav'/><span id='qhconfig_checkbox_text_fav' style='display:inline-block;position:relative;bottom:25%;'>收藏" + get.translation(name) + "</span></span></p>";

              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('forbidImage', 'extension/时装回廊/image/newui_forbid.png') + "' style='width:50px'/>禁用设置</h2>";
              content += "<p>可以选择在某模式下禁用或启用该武将。该设置将在重启游戏后生效。</p>"
              content += "<p><span style='display:inline-block;height:30px;'><img id='qhconfig_checkbox_banned_mode_all'/><span id='qhconfig_checkbox_text_all' style='display:inline-block;position:relative;bottom:25%;'>所有模式禁用</span></span></p>";
              for (var mode in lib.mode) {
                if (mode != 'connect') {
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('rankImage', 'extension/时装回廊/image/newui_rank_icon.png') + "' style='width:50px'/>等阶设置</h2>";
              content += "<p>可以设置" + get.translation(name) + "的等阶，重启后生效。</p>";
              content += "<p><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_rank_select'></select></p>";

              if (lib.config.qhly_enableCharacterMusic) {
                content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('musicImage', 'extension/时装回廊/image/newui_music_icon.png') + "' style='width:50px'/>音乐设置</h2>";
                content += "<p>可以设置" + get.translation(name) + "的专属背景音乐，在游戏开始时将自动切换。</p>";
                content += "<p><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_music_select'></select></p>";
              }
              var extraConfigs = [];
        ...
        subView.pageButton[page].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/时装回廊/image/newui_button_selected.png'));
        view.appendChild(subView.mp);
        var state = {
          name: name,
          currentPage: page,
        ...
        refreshRank = function () {
          if (subView.rank) {
            if (lib.config.qhly_showrarity) {
              ...
              var rarity = game.getRarity(state.name);
              if (rarity) {
                subView.rank.setBackgroundImage('extension/时装回廊/image/rarity_' + rarity + ".png");
              }
            } else {
              subView.rank.hide();
            }
        ...
        var showPage = function (pagename) {
          ...
          for (var k in subView.pageButton) {
            if (k == pagename) {
              subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/时装回廊/image/newui_button_selected.png'));
            } else {
              subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonImage, 'extension/时装回廊/image/newui_button.png'));
            }
          }
        };
        state.useLihui = function () {
          ...
          if (use) {
            if (currentViewSkin.isLolBigLayout) {
              if (state.currentPage == 'skin') {
                ...
                subView.board.setBackgroundImage("extension/时装回廊/theme/lolbig/newui_lol_bg_center_big.png");
                ...
                subView.board.setBackgroundImage("extension/时装回廊/theme/lolbig/newui_lol_bg1_big.png");
            ...
            if (currentViewSkin.isLolBigLayout) {
              if (state.currentPage == 'skin') {
                ...
                subView.board.setBackgroundImage("extension/时装回廊/theme/lolbig/newui_lol_bg_center_small.png");
                ...
                subView.board.setBackgroundImage("extension/时装回廊/theme/lolbig/newui_lol_bg1.png");
                ...
                if (isNaN(hp1) || isNaN(hp2)) {
                  ...
                  if (hp2 >= 6) {
                    ...
                    for (var i = 0; i < hp2; i++) {
                      ...
                      if (i >= hp1) {
                        if (currentViewSkin.isQiLayout) {
                          img.setBackgroundImage('extension/时装回廊/theme/shuimo/newui_shuimo_hpimg_gray.jpg');
                        } else if (currentViewSkin.isLolBigLayout) {
                          img.setBackgroundImage('extension/时装回廊/theme/lolbig/newui_lol_hpimg_gray.png');
                        }
                      }
                    }
                    if (hujia) {
          ...
          if (viewAbstract.skin) {
            ...
            if (title) {
              ...
              if (['精品', '史诗', '传说', '限定'].contains(title)) {
                ...
                levelText.innerHTML = '';
                levelText.setBackgroundImage('extension/时装回廊/image/level_' + obj[title] + ".png");
              } else {
          ...
          if (skills) {
            ...
            for (var skill of skills) {
              ...
              if (window.qhly_TrySkillAudio) {
                infoString += "<a style='color:#ffffff' href=\"javascript:window.qhly_TrySkillAudio('" + skill + "',{name:'" + name + "'},null,window.qhly_audio_which[\'" + skill + "\'],\'" + viewAbstract.skin + "\');window.qhly_audio_which[\'" + skill + "\']++;\"><img style=height:22px src=" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/时装回廊/image/qhly_pic_playaudiobutton.png') + "></a><br>";
              }
              infoString += "<br><br>";
              str += infoString;
            }
          ...
          Object.defineProperty(ui.click, 'charactercard', {
            ...
            set: function (value) {
              if (!lib.config.qhly_mentionConflitCC) {
                var ret = confirm("你安装的扩展中，有扩展试图修改ui.click.charactercard，此行为与《时装回廊》冲突，你可以关闭有冲突的功能。若你点击【取消】，将不再对此消息进行提示。");
                if (!ret) {
      ...
      get.nodeintro=function(node,simple,evt){
          ...
          if(node.name){
              if(get.character(node.name)){
                  ...
                  zhu.innerHTML = "<img style=width:30px src="+lib.assetURL+"extension/时装回廊/qhly_skin_bt1.png>";
                  zhu.listen(function(){
          ...
          if(node.name2 && get.character(node.name2)){
              ...
              fu.innerHTML = "<img style=width:30px src="+lib.assetURL+"extension/时装回廊/qhly_skin_bt2.png>";
              fu.listen(function(){
                ...
                game.qhly_getSkinModels(n, function (list) {
                  if (list && list.length) {
                    ...
                    if (!_status.qhly_open && !_status.bigEditing && !_status.qhly_playerWindowing) game.qhly_setCurrentSkin(n, list.randomGet(), function () {
                      ...
                      if (player && lib.config['extension_时装回廊_qhly_decadeChangeEffect'] && (lib.config.qhly_currentViewSkin == 'decade' || lib.config.qhly_currentViewSkin == 'shousha')) player.playChangeSkinEffect(player.name2 && player.name2 == n);
                    }, true);
                    game.qhly_autoChangeSkin();
                  } else {
                    f(arr, f);
      ...
      game.qhly_checkFileExist('extension/时装回廊/image/diylevels', function (s) {
        if (s && game.getFileList) {
          game.getFileList('extension/时装回廊/image/diylevels', function (folders, files) {
            if (files) {
              lib.qhly_diylevels = {};
              for (var f of files) {
      ...
      game.qhly_checkFileExist('extension/时装回廊/music/', function (s) {
        if (s && game.getFileList) {
          game.getFileList('extension/时装回廊/music/', function (folders, files) {
            if (files) {
              for (var f of files) {
                if (f.endsWith('.mp3')) {
                  var path = 'extension/时装回廊/music/' + f;
      ...
      game.getFileList('extension/时装回廊/plugins', function (folders, files) {
        if (files) {
          for (var file of files) {
            if (file.endsWith('.js')) {
...
//-----Q-----END-----
      ...
      lib.qhly_viewskin['shuimo'] = {
        ...
        buttonImage: 'extension/时装回廊/theme/shuimo/newui_button_shuimo.png',
        buttonPressedImage: 'extension/时装回廊/theme/shuimo/newui_button_selected_shuimo.png',
        skillPagePlayAudioButtonImage: 'extension/时装回廊/theme/shuimo/newui_playaudio_shuimo.png',
        skinPagePlayAudioButtonImage: 'extension/时装回廊/theme/shuimo/qhly_pic_playaudiobutton_shuimo.png',
        onchange: function () {
      ...
      if ((lib.config.qhly_currentViewSkin == 'decade' || lib.config.qhly_currentViewSkin == 'shousha') && !lib.config['extension_时装回廊_qhly_decadeCloseDynamic']) window._qhlyThunder = true;//用于判断是否打开的是时装回廊雷修版
      window._qhlyThunderKey = ['init', 'uninit', 'reinit', 'playDynamic', 'stopDynamic', 'showCharacter'];
      lib.qhly_path = lib.assetURL + 'extension/时装回廊/';
      window.qhly_version = 5;
      var cssUrl = lib.assetURL + 'extension/时装回廊';
      ...
      lib.init.js(lib.qhly_path, 'skinChange', function () {
        let str = '时装回廊：检测到skinChange.js中';
      ...
      "qhly_newui":{
          "name":"新UI",
          "intro":"打开此选项，将使用新版角完UI。",
          "init":lib.config.qhly_newui === undefined ? true:lib.config.qhly_newui,
          onclick:function(item){
              game.saveConfig('extension_时装回廊_qhly_newui',item);
      ...
      "qhly_layoutFitX": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_layoutFitX', item);
      ...
      "qhly_layoutFitY": {
        ...
        "init": lib.config.qhly_layoutFitY === undefined ? true : lib.config.qhly_layoutFitY,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_layoutFitY', item);
      ...
      "qhly_vMiddle": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_vMiddle', item);
      ...
      "qhly_fontsize1": {
        ...
        "init": lib.config.qhly_fontsize1 === undefined ? "3" : lib.config.qhly_fontsize1,
        "item": {
          "1": "很微小",
          "2": "微小",
          "3": "较小",
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_fontsize1', item);
      ...
      "qhly_replaceCharacterCard2": {
        "name": "替换默认资料卡",
        "intro": "打开此选项，将使用时装回廊的资料卡替换角完杀默认的资料卡。",
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_replaceCharacterCard2', item);
      ...
      "qhly_nolihuiOrigin": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_nolihuiOrigin', item);
          game.saveConfig('qhly_nolihuiOrigin', item);
        }
      },
      "qhly_forbidExtPage": {
        "name": "禁用附加页功能",
        "intro": "打开此选项，在角完资料页将无法通过点击【简介】访问附加页面，也不会有小箭头。",
        "init": lib.config.qhly_forbidExtPage === undefined ? false : lib.config.qhly_forbidExtPage,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_forbidExtPage', item);
      ...
      "qhly_titlereplace": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_titlereplace', item);
      ...
      "qhly_recordWin": {
        "name": "展示战绩",
        "intro": "打开此选项，可以在角完资料页查看战绩。",
        "init": lib.config.qhly_recordWin === undefined ? false : lib.config.qhly_recordWin,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_recordWin', item);
      ...
      "qhly_randskin": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_randskin', item);
          game.saveConfig('qhly_randskin', item);
        }
      },
      "qhly_extcompat": {
        "name": "扩展兼容",
        "intro": "打开此选项，时装回廊将在一定程度上兼容大部分带有阵亡配音的扩展。如果不玩扩展武将，关闭此选项可提升性能。",
        "init": lib.config.qhly_extcompat === undefined ? true : lib.config.qhly_extcompat,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_extcompat', item);
      ...
      "qhly_lutou": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_lutou', item);
      ...
      "qhly_lutouType": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_lutouType', item);
      ...
      "qhly_showrarity": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_showrarity', item);
      ...
      "qhly_name_pattern":{
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_name_pattern', item);
      ...
      "qhly_originSkinPath": {
        "name": "本体武将皮肤目录",
        "intro": "可设置本体武将的皮肤目录。",
        "init": lib.config.qhly_originSkinPath === undefined ? "extension/时装回廊/sanguoskin/" : lib.config.qhly_originSkinPath,
        "item": {
          "extension/时装回廊/sanguoskin/": "时装回廊目录",
          "extension/时装回廊/sanguolutouskin/": "角完露头目录",
          "image/skin/": "本体目录",
        },
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_originSkinPath', item);
      ...
      "qhly_extSkinPath": {
        ...
        "init": lib.config.qhly_extSkinPath === undefined ? "extension/时装回廊/sanguoskin/" : lib.config.qhly_originSkinPath,
        "item": {
          "default": "扩展skin文件夹",
          "extension/时装回廊/sanguoskin/": "时装回廊目录",
          "extension/时装回廊/sanguolutouskin/": "角完露头目录",
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_extSkinPath', item);
      ...
      "qhly_listdefaultpage": {
        "name": "列表进入默认页面",
        "intro": "可设置通过武将列表进入时装回廊目录时，默认显示的页面。",
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_listdefaultpage', item);
          game.saveConfig('qhly_listdefaultpage', item);
        }
      },
      "qhly_doubledefaultpage": {
        "name": "双击默认页面",
        "intro": "可设置通过在游戏内双击武将头像进入时装回廊目录时，默认显示的页面。",
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_doubledefaultpage', item);
      ...
      "qhly_guozhan": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_guozhan', item);
      ...
      "qhly_skinconfig": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_skinconfig', item);
          game.saveConfig('qhly_skinconfig', item);
        }
      },
      "qhly_editmode": {
        "name": "编辑模式",
        "intro": "打开此选项后，在角完详情页可以编辑武将台词。",
        "init": lib.config.qhly_editmode === undefined ? false : lib.config.qhly_editmode,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_editmode', item);
      ...
      "qhly_skillingame": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_skillingame', item);
      ...
      "qhly_keymarkopen": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_keymarkopen', item);
      ...
      "qhly_chooseButtonOrigin":{
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_chooseButtonOrigin', item);
      ...
      "qhly_decadeCloseDynamic": {
        ...
        onclick: function (item) {
          if (item === true) {
            ...
            if (!window.decadeUI) {
              alert("侦测到UIPro未正常开启，无法使用动皮功能！");
              lib.config['extension_时装回廊_qhly_decadeCloseDynamic'] = true;
              this.classList.add('on');
              return;
            }
          }
          game.saveConfig('extension_时装回廊_qhly_decadeCloseDynamic', item);
      ...
      "qhly_circle_top":{
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_circle_top', item);
      ...
      "qhly_decadeDengjie": {
        "name": "小窗人物等阶",
        "init": lib.config['extension_时装回廊_qhly_decadeDengjie'] === undefined ? "auto" : lib.config['extension_时装回廊_qhly_decadeDengjie'],
        "item": {
          "one": "一阶",
          "two": "二阶",
          "three": "三阶",
      ...
      "qhly_decadeAuto": {
        "name": "自动换肤间隔",
        "intro": "调节自动换肤时间",
        "init": lib.config['extension_时装回廊_qhly_decadeAuto'] === undefined ? "30" : lib.config['extension_时装回廊_qhly_decadeAuto'],
        "item": {
          "10": "每10秒",
          "30": "每半分钟",
          "60": "每1分钟",
      ...
      "qhly_closeVoice": {
        "name": "关闭点击音效",
        "intro": "打开此选项，可关闭点击音效。",
        "init": lib.config.qhly_closeVoice === undefined ? true : lib.config.qhly_closeVoice,
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_closeVoice', item);
      ...
      "qhly_enableCharacterMusic": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_enableCharacterMusic', item);
      ...
      "qhly_hanggaoxiufu": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_hanggaoxiufu', item);
      ...
      "qhly_hanggaoxiufu2": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_hanggaoxiufu2', item);
      ...
      "qhly_shilizihao": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_shilizihao', item);
      ...
      "qhly_lihuiSupport": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_lihuiSupport', item);
      ...
      "qhly_hideShuimoCover": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_hideShuimoCover', item);
      ...
      "qhly_funcLoadInPrecontent": {
        ...
        onclick: function (item) {
          game.saveConfig('extension_时装回廊_qhly_funcLoadInPrecontent', item);
      ...
      "qhly_restore": {
        ...
        onclick: function () {
          if (lib.config.qhly_save_offical_skin) {
            ...
            game.saveConfig('change_skin', false);
            game.saveConfig('extension_时装回廊_enable', true);
            game.reload();
          }
        }
      },
